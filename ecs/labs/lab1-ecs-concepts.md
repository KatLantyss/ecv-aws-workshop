...
Amazon Elastic Container Service（ECS）是 AWS 提供的**全託管容器編排服務**，讓你可以輕鬆地在 AWS 上執行、停止和管理 Docker 容器。
...
ECS 的架構由以下四個核心元件組成，理解它們之間的關係是掌握 ECS 的關鍵：
...
- 容器的**藍圖 / 設計圖**，類似 `docker-compose.yml`
- 定義內容包含：
  - 使用哪個 Docker Image
  - CPU / Memory 配置
  - Port Mapping
...
- **零基礎設施管理**：不需要 SSH 進入 EC2、不需要 patch OS
- **精確計費**：只為實際使用的 vCPU 和 Memory 付費
- **安全隔離**：每個 Task 運行在獨立的 micro-VM 中
...
> 💡 **簡單判斷**：如果你的團隊沒有 Kubernetes 經驗，且應用主要在 AWS 上運行，ECS 是更簡單高效的選擇。

---

## 1.5 本工作坊的部署流程

以下是我們在後續 Lab 中會依序完成的步驟：
...
1. Task Definition 和 Task 的關係是什麼？
2. Service 的主要職責是什麼？
3. Fargate 模式下，誰負責管理底層的伺服器？
4. ECS Cluster 是什麼？

<details>
<summary>📝 參考答案</summary>

1. Task Definition 是藍圖，Task 是根據藍圖啟動的執行實例
2. 維持指定數量的 Task 持續運行，異常時自動替補
3. AWS 負責，使用者不需要管理任何 EC2 實例
4. ECS 資源的邏輯分組，用來組織和管理 Service 與 Task

</details>

---

前往 [Lab 2 - 部署基礎環境與 ECS 叢集建置](./lab2-cluster-setup.md) ▶
