...
本工作坊提供了一份 CloudFormation 模板 [::button[ecs-workshop-infra.yaml]{variant="link" prefix="arrow-down-to-line"}](ecs-workshop-infra.yaml)，它會建立以下資源：
...
| Target Group | ecs-workshop-tg | 目標群組，Health Check Path: / |
...
| Command Host | ecs-workshop-command-host | t3.micro，預裝 Docker、Git，透過 Session Manager 連線 |
...
3. 在 **Specify template** 頁面：
   ...
   - 點擊 ::button[Choose file]{variant="link" prefix="arrow-up-to-line"} 上傳 `ecs-workshop-infra.yaml`
...
5. 在 **Configure stack options** 頁面：
   - 捲到最下方，勾選 ::status[I acknowledge that AWS CloudFormation might create IAM resources with custom names.]{type="info" icon="square-check"}
   - 點擊 ::button[Next]{variant="primary"}
...
:::alert{type="warning"}
資料庫密碼請記住，後續 Task 6 整合 RDS 時會用到。正式環境應使用 AWS Secrets Manager 管理密碼。
:::
...
| ECSClusterName | ECS 叢集名稱 | Task 4 建立 Service |
| ECRRepositoryUri | ECR 映像儲存庫 URI | Task 3 推送映像 |
| ALBDnsName | ALB DNS 名稱 | 驗證應用程式 |
| S3BucketName | S3 Bucket 名稱 | Task 5 S3 整合 |
| RDSEndpoint | RDS 連線端點 | Task 6 RDS 整合 |
| ECSTaskExecutionRoleArn | 執行角色 ARN | Task 4 Task Definition |
| ECSTaskRoleArn | 任務角色 ARN | Task 4 Task Definition |
| PublicSubnet1 / PublicSubnet2 | 公有子網路 ID | Task 4 建立 Service |
| ECSSecurityGroup | ECS 安全群組 ID | Task 4 建立 Service |
| TargetGroupArn | 目標群組 ARN | Task 4 建立 Service |
