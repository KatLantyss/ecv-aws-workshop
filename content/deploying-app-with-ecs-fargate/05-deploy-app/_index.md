...
透過 ECS Console 建立 Task Definition 與 Service，將 2048 遊戲部署到 Fargate，並透過個人 ALB 存取。
...
5. 在 **Container - 1** 區塊：
   - **Name**：輸入 ``web2048``
   - **Image URI**：貼上 ECR Repository URI + ``:{{USERNAME}}-nginx``（例如 `123456789012.dkr.ecr.us-east-1.amazonaws.com/ecs-fargate-lab-app:{{USERNAME}}-nginx`）
   - **Container port**：``80``
   - **Protocol**：`TCP`
...
Service 會確保指定數量的 Task 持續運行，並與個人 ALB 整合分配流量。
...
3. 在 **Service details** 區塊：
   ...
   - **Task definition revision**：選擇有 **LATEST** 的版本
...
6. 在 **Networking** 區塊：
   ...
   - **Security group**：選擇 **Use an existing security group** → 選擇 `ecs-fargate-lab-infra-ECSSecurityGroup-*`
...
7. 在 **Load balancing** 區塊：
   ...
   - **Use an existing load balancer**：選擇 `ecs-fargate-lab-{{USERNAME}}-alb`
...
:::steps
1. 建立 Service 後點擊 `ecs-fargate-lab-{{USERNAME}}-service` 進入到 Service 詳情頁
2. 切換到 **Deployments** 分頁，觀察部署進度
3. 切換到 **Tasks** 分頁，等待 **Deployment status** 的狀態變為 ::status[Success]{type="success" icon="aws-success"}
:::
...
## 4.4 透過個人 ALB 玩 2048 遊戲

:::steps
1. 在 CloudFormation Outputs 中，複製 **ALBDnsName** 的值
2. 在瀏覽器開啟 `http://<ALB_DNS>`
3. 預期看到 2048 遊戲畫面，可用方向鍵試玩
:::

:::alert{type="success"}
2048 遊戲已成功部署至 AWS Fargate，透過個人 ALB 提供服務。
:::
...
| 項目 | 驗證方式 | 預期結果 |
|------|----------|----------|
| Task Definition | ECS Console → Task definitions | `ecs-fargate-lab-{{USERNAME}}-app:*` |
| Service 狀態 | ECS Console → Cluster → Services | Running tasks: 2 |
| 2048 遊戲 | 瀏覽器開啟個人 ALB DNS | 看到 2048 遊戲畫面 |
