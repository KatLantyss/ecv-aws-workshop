...
講師已預先部署共用基礎設施（VPC、ECS Cluster、ECR、RDS）。本步驟將部署個人 Lab 資源，包含 Command Host、S3 Bucket 和 ALB。
...
| ALB | ecs-fargate-lab-{{USERNAME}}-alb | 個人負載平衡器 |
...
:::alert{type="warning"}
**UserPrefix** 務必填入個人 username ``{{USERNAME}}``，避免與其他學員衝突。
:::
...
| ALBDnsName | 個人 ALB DNS 名稱 | Task 4 瀏覽器開啟驗證應用 |
| S3BucketName | 個人 S3 Bucket 名稱 | Task 5 上傳靜態檔案 |
| RDSEndpoint | RDS 連線端點 | Task 5 設定資料庫連線 |
| ECSClusterName | ECS Cluster 名稱 | Task 4 建立 Service |
| ECSSecurityGroup | ECS 安全群組 ID | Task 4 建立 Service 的 Networking |
| PublicSubnet1 / PublicSubnet2 | Public Subnet ID | Task 4 建立 Service 的 Networking |
